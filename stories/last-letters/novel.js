const SAVE_KEY = "last-letters-save-v1";
...
const traitProfiles = {
  harinTrust: {
    label: "하린의 신뢰",
    summary: "서윤은 하린의 침묵을 재촉하기보다, 그가 말할 수 있는 범위를 함께 지키려 합니다.",
  },
  minjaeTrust: {
    label: "민재의 신뢰",
    summary: "서윤은 민재의 기다림을 회피가 아니라, 함께 감당해야 할 약속으로 읽기 시작합니다.",
  },
  boundary: {
    label: "경계",
    summary: "서윤은 사실을 남기되, 살아 있는 사람의 편지와 선택까지 대신 설명하지 않으려 합니다.",
  },
  urgency: {
    label: "서두름",
    summary: "서윤은 침묵이 남긴 시간을 더 늘리고 싶지 않아, 먼저 움직이는 쪽으로 기웁니다.",
  },
  guilt: {
    label: "죄책감",
    summary: "서윤은 받지 못한 전화 한 통을 오래 붙들고 있으며, 그 감정이 다른 책임을 대신하지 않도록 배우고 있습니다.",
  },
};
...
async function renderStory() {
  ...
  try {
    ...
    if (renderVersion !== storyRenderVersion || story.scenes[state.sceneId] !== scene) return;
    const additions = getSceneAdditions(scene).map((line) => `<p class=\"conditional-text\">${escapeHtml(line)}</p>`).join("");
    elements.story.innerHTML = `${ending}${markdownToParagraphs(body)}${additions}`;
    renderChoices(scene);
  } catch (error) {
    if (renderVersion !== storyRenderVersion) return;
    elements.story.innerHTML = `<p>본문을 불러오지 못했습니다. 연결을 확인한 뒤 다시 열어 주세요.</p><p class=\"error-detail\">${escapeHtml(error.message)}</p>`;
  }
}
...
function renderRelationships() {
  ...
  if (introducedCharacters.length === 0) {
    elements.relationshipList.innerHTML = `
      ...
      <p class="empty-state">아직 서윤이 관계를 다시 읽기 시작한 인물은 없습니다.</p>
    `;
    return;
  }
...
function renderMemory() {
  if (state.memory.length === 0) {
    elements.memoryList.innerHTML = `<p class="empty-state">아직 서윤에게 남은 문장이나 단서는 없습니다.</p>`;
    return;
  }
...
const memorySections = [
  {
    id: "sentence",
    title: "남은 문장",
    description: "서윤이 끝내 지우지 못한 마음과 반응입니다.",
...
function memoryFallbackDetail(item) {
  if (item.type === "structural") {
    return "이 선택 이후 서윤이 먼저 보게 되는 진실이 달라졌다.";
  }

  if (item.type === "illusion") {
    return "사건은 크게 움직이지 않았지만, 서윤의 반응은 남았다.";
  }
